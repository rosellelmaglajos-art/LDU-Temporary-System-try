...
export default function Forms({ view }) {
  ...
  const [modalType, setModalType] = useState('list'); // 'list', 'create_nomination', 'create_ja', 'view'
                ...
                forms.filter(f => filter === 'all' || f.status.includes(filter)).map(form => (
                  <tr key={form._id} className="hover:bg-slate-50/50 transition-colors group">
                    ...
                    <td className="px-6 py-4 text-right">
                      <button onClick={() => { setSelectedForm(form); setModalType('view'); setShowModal(true); }} className="p-2 text-slate-400 hover:text-[#0b3d91] hover:bg-blue-50 rounded-lg transition-colors">
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 sm:p-6 animate-in fade-in duration-200">
          <div className="bg-white rounded-3xl w-full max-w-4xl max-h-[90vh] flex flex-col shadow-2xl overflow-hidden animate-in slide-in-from-bottom-8">
            <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50">
              <div>
                <h3 className="text-xl font-extrabold text-slate-900 capitalize">{selectedForm.type.replace('_', ' ')} Details</h3>
                <p className="text-sm text-slate-500 font-medium">Submitted by {selectedForm.user_name}</p>
              </div>
              <div className="flex items-center gap-3">
                <button onClick={generatePDF} className="flex items-center gap-2 px-4 py-2 bg-slate-200 text-slate-700 hover:bg-slate-300 font-bold rounded-xl transition-colors">
            ...
            <div className="p-6 overflow-y-auto flex-1 bg-slate-50/50">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                {Object.entries(selectedForm.data).map(([k, v]) => (
            ...
            {view === 'approvals' && (
              <div className="p-6 border-t border-slate-200 bg-white flex gap-4 justify-end">
                <button onClick={() => handleStatusUpdate(selectedForm._id, 'rejected')} className="px-6 py-3 font-bold text-red-600 bg-red-50 hover:bg-red-100 rounded-xl transition-colors">
                  Reject
                </button>
                <button onClick={() => handleStatusUpdate(selectedForm._id, user.role === 'Supervisor' ? 'pending_hrdd' : 'approved')} className="px-6 py-3 font-bold text-white bg-emerald-600 hover:bg-emerald-700 shadow-lg shadow-emerald-200 rounded-xl transition-colors flex items-center gap-2">
